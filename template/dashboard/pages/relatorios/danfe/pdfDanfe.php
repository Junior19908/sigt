...
session_start(); require_once '../../../../../controller/user.php';
if($type == 1){
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Criação dos PDF's</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
...
<div class="card card-success">
    <div class="card-body">
        <!-- Seu código PHP adicionado aqui -->
        <?php
        ...
        // Defina o tempo limite de execução para ilimitado
        set_time_limit(0);

        // Diretório onde estão os arquivos XML
        $xmlDir = 'C:/ArquivosSAP/xml/';
        // Diretório onde serão gerados os PDFs
        $pdfDir = 'C:/ArquivosSAP/xml/pdf/';
        // Lista todos os arquivos XML na pasta
        $xmlFiles = glob($xmlDir . '*.xml');
        
        $logo = 'data://text/plain;base64,' . base64_encode(file_get_contents(realpath('../../../../../css/image/logo-nfe.png')));
        
        foreach ($xmlFiles as $xmlFile) {
            try {
                $caminho_temporario = $xmlFile; // Use o próprio arquivo XML como caminho temporário
                $xml = file_get_contents($caminho_temporario);
                $teste = simplexml_load_string($xml);
                $natOp = (string) $teste->NFe->infNFe->ide->natOp;
                ...
                try {
                  // Crie uma instância da classe PDO para se conectar ao banco de dados
                  $pdo = new PDO("mysql:host=$hostname;dbname=$database", $username, $password);
              
                  // Defina o modo de erro do PDO para exceções
                  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              
                  // Construa uma consulta SQL de inserção
                  $sql = "UPDATE `tb_chave` SET `natOp` = :natOp WHERE col_chave = :chaveAcesso;";
              
                  // Prepare a consulta SQL
                  $stmt = $pdo->prepare($sql);
              
                  // Associe o valor de $natOp ao parâmetro na consulta SQL
                  $stmt->bindParam(':natOp', $natOp, PDO::PARAM_STR);
                  $stmt->bindParam(':chaveAcesso', basename($xmlFile, '.xml'), PDO::PARAM_STR);

                  // Execute a consulta SQL
                  $stmt->execute();
              
                  // A inserção foi bem-sucedida
                  //echo "Natureza da Operação ". $natOp ." inserido no banco de dados com sucesso.";
              } catch (PDOException $e) {
                  // Ocorreu um erro na conexão ou na consulta SQL
                ...
                $danfe = new Danfe($xml);
                // Resto do seu código para configurar o Danfe
                //var_dump($danfe);
                $pdf = $danfe->render($logo);

                // Nome do arquivo PDF (usando o mesmo nome do XML, mas com extensão PDF)
                $pdfFileName = $pdfDir . basename($xmlFile, '.xml') . '.pdf';
                
                //var_dump(basename($xmlFile, '.xml'));
                // Salva o PDF na pasta de destino
                file_put_contents($pdfFileName, $pdf);

                // Exclui o arquivo XML após a geração do PDF
                unlink($caminho_temporario);

                //echo "PDF gerado com sucesso: $pdfFileName<br>";
                  ...
                  <div class='card-body p-0'>
                    <div class='table-responsive'>
                      <table class='table m-0'>
                        ...
                        <tbody>
            ";
            echo "<td><center>$pdfFileName</center></td>";
            } catch (InvalidArgumentException $e) {
                http_response_code(500); // Erro interno do servidor
                echo "Erro ao processar o arquivo $xmlFile: " . $e->getMessage() . "<br>";
            }
        }
        ?>
